...
- **Configurable table, schema and collection names.** The stores no longer hardcode `Settings`, `SettingAudits` and `settings`, which blocked shared databases and installations whose schema already used those names. Set them through `SettingTableOptions` on `UseDapper(...)`, on `ApplySettingsConfiguration(...)` for EF Core, or with `collectionName:` for MongoDB; index names follow the table name so two settings tables can share a schema. Defaults are unchanged. Because table and schema names are composed into the statement text rather than passed as parameters, they are validated as plain SQL identifiers at startup and anything else is rejected.
