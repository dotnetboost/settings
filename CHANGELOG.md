...
- Publishing to nuget.org uses Trusted Publishing (OIDC) instead of a stored API key — the workflow exchanges a GitHub identity token for a credential valid one hour, so there is no long-lived secret in the repository. Scoped to the `nuget-release` environment, which makes required reviewers on that environment an actual publish gate.
